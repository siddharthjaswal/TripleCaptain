...
type DifferentialPlayer = {
  ...
  photo: string | null;
  price: number;
  ownership: number; // Low ownership = differential
  form: number;
  pointsPerGame: number;
  expectedPoints: number;
  upcomingFixtureDifficulty: number; // 1-5
  differentialScore: number; // 0-100, higher = better differential
};
    ...
    <div className="tc-card overflow-hidden">
            ...
            {topDifferentials.map((player) => {
              ...
              return (
                <div
                  ...
                  className="rounded-xl border bg-gradient-to-br from-purple-500/5 to-pink-500/5 border-purple-500/20 hover:border-purple-500/40 p-4 transition-all"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-black ${
                          ownershipTier.color === 'purple' 
                            ? 'bg-purple-500/20 text-purple-600 dark:text-purple-400 border border-purple-500/30'
                            : ownershipTier.color === 'blue'
                      ...
                      <div className="flex items-center gap-3 text-xs">
                        <div className="flex items-center gap-1">
                          <span className="tc-text-muted">Form:</span>
                          <span className="font-black text-blue-500">{player.form}</span>
                        </div>
                        <span className="tc-text-muted">•</span>
                        <div className="flex items-center gap-1">
                          <span className="tc-text-muted">Pts/Game:</span>
                          <span className="font-black">{player.pointsPerGame.toFixed(1)}</span>
                        </div>
                        <span className="tc-text-muted">•</span>
                        <div className="flex items-center gap-1">
                          <span className="tc-text-muted">FDR:</span>
                          <span className={`font-black ${getDifficultyColor(player.upcomingFixtureDifficulty)}`}>
                            {player.upcomingFixtureDifficulty}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="shrink-0 text-center">
                      <div className="w-16 h-16 rounded-full bg-gradient-to-br from-purple-500/20 to-pink-500/20 border-2 border-purple-500/30 flex items-center justify-center">
                        <div>
                          <p className="text-xl font-black text-purple-600 dark:text-purple-400 leading-none">
                            {player.differentialScore}
                          </p>
                          <p className="text-[8px] font-bold tc-text-muted uppercase">Score</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-[color:var(--surface-border)] flex items-start gap-2">
                    ...
                    <p className="text-xs tc-text-muted">
                      <span className="font-bold">Why pick?</span> {getReasonText(player)}
                    </p>
                  </div>
                </div>
              );
      ...
      <div className="border-t border-[color:var(--surface-border)] p-3 bg-[color:var(--surface-root)]">
        <p className="text-xs tc-text-muted text-center">
          💎 Differentials can give you an edge in mini-leagues • Risk vs Reward
        </p>
      </div>
    </div>
  );
...
function getReasonText(player: DifferentialPlayer): string {
  const reasons = [];
